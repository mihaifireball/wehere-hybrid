...
class WeHereConfigFlow(
    ...
    async def async_step_user(
        ...
        ok = await WeHereCloud.request_verification_code(
            self.hass,
            self.email,
        )
    ...
    async def async_step_verify(
        ...
        self.devices = await WeHereCloud(
            self.hass,
            temp_entry,
        ).get_devices()
    ...
    async def async_step_device(
        ...
        if not user_input.get("skip_device"):
            ...
            cfg[CONF_MAC_ADDRESS] = (
                user_input[CONF_MAC_ADDRESS]
                .replace(":", "")
                .upper()
            )
            ...
            voltage = await WeHereCloud(
                self.hass,
                temp_entry,
            ).get_voltage_config(
                cfg.get("deviceType"),
                cfg.get("hardwareVersion"),
            )

            cfg[CONF_VOLTAGE_THRESHOLDS] = (
                [
                    float(
                        voltage[f"fvoltage{i}"]
                    )
                    for i in range(1, 5)
                ]
                if voltage
    ...
    async def async_step_reauth(
        ...
        self.email = self._reauth_entry.data.get(
            CONF_EMAIL
        )
    ...
    async def async_step_reauth_verify(
        ...
        """Verify a new authentication code."""
        if user_input is None:
            return self.async_show_form(
                step_id="reauth_verify",
                data_schema=vol.Schema(
