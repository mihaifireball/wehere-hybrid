...
class WeHereCloud:
    ...
    def _check_auth(data):
        """Raise when the WeHere token is no longer valid."""
        if data.get("code") == 5:
            raise ConfigEntryAuthFailed(
                data.get("info") or "WeHere authentication expired"
            )
    ...
    async def request_verification_code(hass, email):
        ...
        try:
            ...
            _LOGGER.exception(
                "WeHere verification-code request failed"
            )
            return False
    ...
    async def get_devices(self, user_id=None, token=None):
        ...
        url = (
            f"{BASE_URL}/api/v2/lock/getAllDevicesNew"
            f"?language={LANGUAGE}"
            f"&userId={user_id}"
            f"&version={VERSION}"
            f"&token={token}"
    ...
    async def get_voltage_config(
        ...
        url = (
            f"{BASE_URL}/api/lock/getAllInfo1"
            f"?language={LANGUAGE}"
            f"&userId={user_id}"
            f"&version={VERSION}"
            f"&token={token}"
        ...
        try:
            ...
            _LOGGER.exception(
                "WeHere voltage configuration lookup failed"
            )
    ...
    async def _request(
        ...
        raise RuntimeError(
            str(last)
            if last
            else "WeHere cloud request failed"
        )
    ...
    async def operate_lock(
        ...
        if not gateway:
            raise RuntimeError(
                f"No gateway found for lock {sn}"
            )

        mark = "1" if unlock else "2"

        url = (
            f"{BASE_URL}/api/lock/lockOrUnlockChildDevice"
            f"?language={LANGUAGE}"
            f"&sn={gateway}"
            f"&userId={self.entry.data[CONF_USERID]}"
            f"&uuid={uuid.uuid4()}"
            f"&version={VERSION}"
            f"&mark={mark}"
            f"&childDeviceSn={sn}"
            f"&token={self.entry.data[CONF_TOKEN]}"
        )
        ...
        if res.status_code != 200:
            raise RuntimeError(
                f"Cloud HTTP error {res.status_code}"
            )
